...
define(['js/Constants',
    ...
    './BusinessProcessModelingBasePool',
    './BusinessProcessModelingBaseLane',
    ...
    */
    ...
    BusinessProcessModelingDecoratorCore.prototype._renderContent = function () {
        ...
        var META_TYPES = BusinessProcessModelingMETA.getMetaTypes(),
            ...
            isTypeEventBasedG = BusinessProcessModelingMETA.TYPE_INFO.isEventBasedG(gmeID),
            isTypeTaskBasedG = BusinessProcessModelingMETA.TYPE_INFO.isTaskBasedG(gmeID),
            ...
            isAbstract = gmeID === META_TYPES.EventBasedG || gmeID === META_TYPES.TaskBasedG || gmeID === META_TYPES.Event || gmeID === META_TYPES.Activity || gmeID === META_TYPES.Artifact || gmeID === META_TYPES.Pool || gmeID === META_TYPES.Lane,
            isTypeWithPorts = BusinessProcessModelingMETA.TYPE_INFO.isPool(gmeID);
        ...
        /*if ((isTypeArtifact || isTypeActivity || isTypeEvent || isTypeTaskBasedG || isTypeEventBasedG || isTypePool || isTypeLane) && !isAbstract) { 
        ...
        if (isTypePool) {
            ...
            _.extend(this, new BusinessProcessModelingBasePool());
        } else if (isTypeLane){
             _.extend(this, new BusinessProcessModelingBaseLane());
        } else if ((isTypeArtifact || isTypeActivity || isTypeEvent || isTypeTaskBasedG || isTypeEventBasedG) && !isAbstract){
           _.extend(this, new BusinessProcessModelingBase()); 
        }
    ...
    BusinessProcessModelingDecoratorCore.prototype._updateName = function () {
        ...
        var control = this._control,
            ...
            desc = node ? node.getAttribute(BPMDConstants.DESC) : '',
                        parentID = node ? node.getParentId() : '',
            ...
            isTypePool = BusinessProcessModelingMETA.TYPE_INFO.isPool(gmeID),
            isTypeLane = BusinessProcessModelingMETA.TYPE_INFO.isLane(gmeID),
            isAbstract = gmeID === META_TYPES.EventBasedG || gmeID === META_TYPES.TaskBasedG || gmeID === META_TYPES.Event || gmeID === META_TYPES.Activity || gmeID === META_TYPES.Artifact,
            isNonAbstract = !isAbstract && (isTypeArtifact || isTypeLane || isTypePool || isTypeActivity || isTypeEvent || isTypeEventBasedG || isTypeTaskBasedG),
            isParentBPModelDiagram = parentID ? BusinessProcessModelingMETA.TYPE_INFO.isBPModel(parentID): false,
        ...
        if (this.skinParts.$name) {
            ...
            if (isTypePool || isTypeLane) {
                    this.skinParts.$name.css('position', 'absolute');
                    this.skinParts.$name.css('top', BPMDConstants.NAME_DIV_TOP);
            }
    ...
    BusinessProcessModelingDecoratorCore.prototype._createNewTextElement = function (newtextheight, x, name) {
        var newtext = document.createElementNS("http://www.w3.org/2000/svg","text");

        newtext.setAttributeNS(null,"x",x);
        newtext.setAttributeNS(null,"y",newtextheight);
        newtext.setAttributeNS(null,"font-size",12); //Textgröße
        newtext.textContent = name || '';
        newtext.setAttributeNS(null,"font-family",'Trebuchet MS');
        return newtext;
    };

    BusinessProcessModelingDecoratorCore.prototype._updateSVGCompartments = function (gmeID, childrenIDs) {
        var self = this,
            ...
            isTypeTask = BusinessProcessModelingMETA.TYPE_INFO.isTask(gmeID),
            isTypeSubProcess = BusinessProcessModelingMETA.TYPE_INFO.isSubProcess(gmeID),
            isTypePool = BusinessProcessModelingMETA.TYPE_INFO.isPool(gmeID),
            isTypeLane = BusinessProcessModelingMETA.TYPE_INFO.isLane(gmeID),            
            isTypeWithCompartments = isTypePool || isTypeLane,
            propertynum = 0,
            props = [],
            isAProperty,
            textheight,
            ...
            x,
            newText,
            BLOCK_HEIGHT_INCREASE = BPMDConstants.CHANGE_HEIGHT,
            MIN_BLOCK_HEIGHT,
            MIN_SVG_HEIGHT,
            high,
            highsvg,
        ...
        for (i = 0; i < childrenIDs.length; i++) {
            ...
            desc = chNode ? chNode.getAttribute(BPMDConstants.DESC) : '',
            isAProperty = BusinessProcessModelingMETA.TYPE_INFO.isTask(ChId) || BusinessProcessModelingMETA.TYPE_INFO.isSubProcess(ChId);
        ...
        for (i = 0; i < props.length; ++i) {
            textheight = 60.23337;
            newtextheight = textheight + i * 15;
            x = isTypeTask ? 6.983763 : 12.983763;
